class PushPull extends Circuit {
	constructor(ctx) {
		super(ctx);
	...
	anodePlot() {
		...
		let vg2 = this.parameters.field[1].value;
		let vg1 = 0.0;
		let k = -1;
		let i = 0;
	...
	loadLines() {
		...
		let vs = this.parameters.field[1].value;
		...
		for (let i = 0; i < 101; i++) {
			let va = i * vaMaxA / 100;
			let ia1 = iaMaxB - va * 4000.0 / raa;
			let ia2 = iaMaxA - va * 2000.0 / raa;
			
			acLoadLine.push({ x: va, y: smax(ia1, ia2, 5) });
		}
		...
		for (let i = 1; i < 101; i++) {
		    let va = this.device.definition.vaMax * i / 100.0;
			let ia = this.device.model.anodeCurrent(va, 0.0, vs) * 1000.0;
